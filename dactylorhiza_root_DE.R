...
draw_heatmap(all_samples_species_locality_treatment, annotation_values = c("species", "treatment"))
draw_heatmap(all_samples_species_treatment_interaction, annotation_values = c("species", "treatment"))

# get sig DE genes for each model fitted
a<-get_significant_genes(all_samples_treatment, mappings_format = TRUE)
b<-get_significant_genes(all_samples_species_treatment, mappings_format = TRUE)
c<-get_significant_genes(all_samples_species_locality_treatment, mappings_format = TRUE)
d<-get_significant_genes(all_samples_species_treatment_interaction, mappings_format = TRUE)
...
# find enriched terms for each model
mp<-readMappings("/Users/katieemelianova/Desktop/Dactylorhiza/data/all_annotations_justGO.txt")
trt_enrich<-get_enriched_terms(a, mp) %>% data.frame() %>% filter(classicFisher < 0.05) %>% dplyr::select(Term, Annotated, Significant, Expected)
sp_trt_enrich<-get_enriched_terms(b, mp) %>% data.frame() %>% filter(classicFisher < 0.05) %>% dplyr::select(Term, Annotated, Significant, Expected)
sp_loc_trt_enrich<-get_enriched_terms(c, mp) %>% data.frame() %>% filter(classicFisher < 0.05) %>% dplyr::select(Term, Annotated, Significant, Expected)
sp_trt_int_enrich<-get_enriched_terms(d, mp) %>% data.frame() %>% filter(classicFisher < 0.05) %>% dplyr::select(Term, Annotated, Significant, Expected)

trt_enrich %>% head(10)
sp_trt_enrich %>% head(10)
sp_loc_trt_enrich %>% head(10)
sp_trt_int_enrich %>% head(10)
